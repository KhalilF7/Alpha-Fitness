...
public class StatistiquesController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        remplirListSeries();
        bcStock.getData().addAll(series);

    }

    private ImageView getImageForProduct(String productName) throws IOException {
        File f = new File(sp.getByNom(productName).getImage().replaceAll("file:/", ""));
        ...
        try {
            ...
            Platform.runLater(() -> {
                series.getData().forEach(seriesData -> {
                    ...
                    try {
                        imageView = getImageForProduct(name);
                    } catch (IOException ex) {
                        Logger.getLogger(ex.getMessage());
                    }
                    imageView.fitWidthProperty().bind(node.widthProperty().multiply(.7));
    ...
    private void SavePDF(ActionEvent event) throws IOException, Exception {
        ...
        try {
            ...
            img1.scaleAbsoluteHeight(328);
            img1.scaleAbsoluteWidth(587);
